...
class wfmf{
	...
	var $js_dir = 'js';	// JavaScript folder name (templates)
	var $js_theme_dir;
	var $js_file_path;
	...
	public function __construct(){
		...
		if(isset($this->settings[$js_file_key]) && is_file(plugins_url($this->settings[$this->js_file_key], __FILE__))):
	...
	public function wfmf_done(){
		...
		if (!is_admin() ):
			if(isset($this->settings[$this->js_file_key]) && !empty($this->settings[$this->js_file_key])):
				foreach($this->js_file_path as $key=>$js_fpath):
					if($key==0)
						$this->js_url = plugins_url('/' . $this->js_dir . '/' .  $this->js_file, __FILE__);
					else if($key==1)
						$this->js_url = plugins_url('/' . $this->js_dir . '/templates/' .  $this->js_file, __FILE__);
					else if($key==2)
						$this->js_url = plugins_url('/' . $this->js_theme_dir . '/' .  $this->js_file, __FILE__);

					if(is_file($js_fpath . '/'. $this->js_file)):
	...
	public function init_settings(){
		...
		$this->js_theme_dir = get_template_directory() . '/js/'. $this->plugin_name;
		$this->js_file_path[0] = plugin_dir_path( __FILE__ ) . '/' . $this->js_dir . '/';
		$this->js_file_path[1] = plugin_dir_path( __FILE__ ) . '/' . $this->js_dir . '/templates/';
		$this->js_file_path[2] = $this->js_theme_dir . '/';
	}
	...
	function admin_settings_page(){
		...
		if(isset($_POST['selected_js_file'])):
			if(empty($_POST['selected_js_file'])):
				...
				foreach($this->js_file_path as $jf_path):
					if(is_file($jf_path . '/' . $js_file)):
		...
		if(is_dir($this->js_theme_dir))
			$scandir = array_merge($scandir, scandir($this->js_theme_dir));		
